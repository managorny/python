...
import math
numbers = [int(i) for i in input("Введите список целых чисел через пробел: ").split()]

i = 0

for value in numbers:
	calcResult = math.sqrt(value)
	if calcResult.is_integer() and (calcResult != number[i]):
		numbers[i] = int(calcResult)
        else:
                numbers.pop(i)
	i += 1
...
numbers = input("Введите в кавычках дату в формате dd.mm.yyyy: ")
...
while n < 0:
        numberRandom = random.randint(-100, 100)
        values.append(numberRandom)
        n = n - 1
